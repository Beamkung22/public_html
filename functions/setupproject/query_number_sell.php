...
<div id="datanumber" style="width:100%;" class="center" role="tabpanel" data-example-id="togglable-tabs">
                      <ul id="myTab" class="nav nav-tabs bar_tabs" role="tablist">
                        ...
                        $num++; ?>
                        <li role="presentation" <?php if($num == $pagenum){ echo "class='active'"; }else{ echo "class=''";} ?>><a href="#tab_content<?php echo $num; ?>" id="home-tab" role="tab" data-toggle="tab" aria-expanded="true"><?php echo $key; ?></a>
                        </li>
                        <?php } ?>
                      </ul>
                      <div id="myTabContent" class="tab-content">
                        ...
                        $numValue = 0;
                          foreach($pageDataShow as $key => $one) {
                            $numValue++;
                            ?>
                            <div role="tabpanel" <?php if($numValue == $pagenum){ echo "class='tab-pane fade active in'"; }else{ echo "class='tab-pane fade'";} ?> id="tab_content<?php echo $numValue; ?>" aria-labelledby="home-tab">
                            <?php
                            foreach($pageDataShow[$key] as $keys => $value) {
                              //if ($pageflagfix[$key][$keys] == "N" 
                              //    ||  ($pageflagfix[$key][$keys] == "Y" && $pagenumberfix[$key][$keys] > 0)){
                              ?>
                              <input type="checkbox" name="A" class="a" value="<?php echo $value; ?>" onclick="sellNumber(this,'<?php echo $numValue; ?>')"  <?php if(isset($_SESSION['dataEditSell'])){ foreach($_SESSION['dataEditSell'] as $keyss => $typeaa) { if($cls_conn->change_type_data($_SESSION['typeSell'][$keyss]) == $type){ if($_SESSION['numberSell'][$keyss] == $value){ echo 'checked';  } } } }?>/>
                              <?php
                              //}
                            }
                            ?>
